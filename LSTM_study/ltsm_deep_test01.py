# -*- coding: utf-8 -*-
"""
Created on Tue Jan 22 04:08:41 2019
...
# version0.2.1 タイムデータに基づくシステム

with open('20120813-210004.csv','rt')as csvfile:
...
pacedata = pd.read_csv('20120813-210004.csv', header =locate_header()[0])  #ファイルを開きます。
print('Load successed')
print('data_size = ',pacedata.shape)

paceR = pacedata[pacedata.subject == 'HUMAN'] #NOISEを除き、人間だけを残します。


#def csv_cutter():#csvファイルを時間的に分ける
...
def draw_plot(start,end,c):#（課題1）図を作成する。直接に数値を入れると、順番に作成する。リストを入れるとリストの順番に作成する。
    ...
    for i in ran:
        ...
        plt.title('ID%s'%id_t)
        plt.savefig('E:\dataset\%d.jpg'%id)
...
def data_get(num):   #リストのリストである型のデータ単純なリスクにします。
    ...
    for j in range(len(x_list)):
        x_value_list = x_list[j]
        x_value = x_value_list[0]
        x_train_list.append(x_value)

    for j in range(len(y_list)):
        y_value_list = y_list[j]
        y_value = y_value_list[0]
        y_train_list.append(y_value)
    
    #train_data=np.array(x)
    #train_x_list=train_data.tolist()    
    x_list=np.reshape(x_list,newshape=(-1,1))
    y_list=np.reshape(y_list,newshape=(-1,1))
    
    return np.array(x_train_list,dtype=np.float32),np.array(y_train_list,dtype=np.float32)
...
def pearson(p1,p2):#ピアソン相関係数を算出します。
    data_std = data_get(p1)    #基準とするデータのｐｘとｐｙを手に入れます。
    x_std = data_std[0]       
    y_std = data_std[1] 
    x_stdf = list_float(x_std,1)
    y_stdf = list_float(y_std,1)
    for i in range(p2,p2+1):   #比べたいデータのｐｘとｐｙを手に入れます。
        data = data_get(i)
        x11 = data[0]
        y11 = data[1]
    x11 = list_float(x11,1)
    y11 = list_float(y11,1)
